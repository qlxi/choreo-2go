...
const UUID = process.env.UUID || '986e0d08-b275-4dd3-9e75-f3094b36fa2a'; // Если нужно изменить UUID, нужно изменить его в config.json на тот же
const NEZHA_SERVER = process.env.NEZHA_SERVER || 'nz.f4i.cn';     
const NEZHA_PORT = process.env.NEZHA_PORT || '5555';                     // Порт Nezha: {443,8443,2096,2087,2083,2053} один из них с включенным tls
const NEZHA_KEY = process.env.NEZHA_KEY || '';   // Если не используете Nezha, оставьте этот параметр пустым
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'choreo.illuminat.ggff.net';     // Рекомендуется использовать token, порт argo 8080, настройки в CF должны соответствовать, при использовании json нужно загрузить json и yml файлы в каталог files
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiOWRhYzAwOTlmMGY3N2NjYjQ3NjNlMTE3YjczZTgxMjAiLCJ0IjoiYmVhMGQ1YTUtZDMwNy00OWY3LTgzZWUtN2QxZDdjNzJjMmMwIiwicyI6Ik1qQmxaamt4WVRBdFpEWmxPQzAwWTJWa0xUa3pNVEF0WVRFMFpHRTBNMlk1WWpRNSJ9';
